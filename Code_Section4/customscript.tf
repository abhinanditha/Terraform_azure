...
resource "azurerm_storage_account" "vmstore45776877" {
  name                     = "vmstore45776877"
...
resource "azurerm_storage_container" "scripts" {
  name                  = "scripts"
  storage_account_name  = "vmstore45776877"
  container_access_type = "blob"
  depends_on=[
    azurerm_storage_account.vmstore45776877
    ]
}
...
resource "azurerm_virtual_machine_extension" "vmextension" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": ["https://${azurerm_storage_account.vmstore45776877.name}.blob.core.windows.net/scripts/Script.ps1"],
